...
**The export read carries two disjoint sets (`D56`).** Alongside the receipts, `exportReceipts` returns the **confirmed donations that are not flagged reportable** — rows the report union (`domain-modeling.md §6`) excludes by definition, and therefore rows with no receipt for a reporter to act on. They travel **beside** the receipts and never among them: folding them in would change what "reported" means and break both `I15`/`I16` and the conservation property `D27` depends on. Each carries `canReport`, which answers `I16(b)`'s source requirement **on the read** so a row can say why it cannot be flagged instead of offering a control the write would reject with a 400. `setReportable` is unchanged and remains the only enforcement — this is the same read-side-mirrors-write-side arrangement as `editWindowOpen` above, and it carries the same obligation: when the write rule changes, the read must move with it.

**S2.3 reads one donation by id (`D76`).** `GET /donations/:id` under the `RECEIVER` gate, replacing `GET /donations` — the pantry-wide worklist that screen used to hold. The list moved to S2.1b's panel and travels on `GET /receive/donations/summary`, whose path and gate are unchanged and whose payload widened. A read by id is what makes the weighing page addressable: the row is fetched rather than handed over by the screen that linked to it, so a reload on a docked tablet lands on the same donation and a 404 is how the page learns another receiver got there first.

**Attaching a store to a label-only walk-in (`D72`).** `POST /report/donations/attach-donor`, under the existing `REPORTER` gate, sets `donor_id` on confirmed donations that carry only a typed-in name. It lives in `services/report.ts` rather than beside `setReportable` in `services/donation.ts`, because what it has to reason about is the *receipt* — `receiptKey` and the `(pickup date, donor)` grouping — not the donation's own lifecycle. The receipt carries `labelDonationIds` so the read can offer the action on precisely the rows it will accept. Three things the service re-checks, none of which the caller can be trusted for: `I29`'s on-route guard, since a donation on a shift may not name a store already stopped at on that shift; that the donor is not archived; and `I16(b)`, which an anonymous row fails. **`ck_ud_source_exclusive` (tier-1) forbids holding both a store and a label**, so the typed name is cleared and appended to the donation's `note`, which `intakeNotes` already carries onto the receipt — the provenance survives on the artefact the reporter actually reads, without a fourth source state nothing else understands.

**A receipt requires *reportable* intake, not merely intake.** A `(pickup date, donor)` whose only intake was a switched-off donation used to produce a fileable card ticked `No Pounds` — reporting to the food bank that a store came to nothing on a day it had never been on the route. The receipt object was built before the reportable filter was applied. Named here rather than only in the service, because it is the boundary between "what we recorded" and "what we tell NTFB", and that boundary is this document's business.
...
- **The receiver edit window** (`PRD` cap 15) — past `app_config.receiver_edit_window_days`, editing an intake row becomes Reporter-only. **The receive payload now reports whether it is open** (`editWindowOpen`, `D47`), so the tablet can hide an affordance that could only be refused. This is the *same predicate the service already gates on*, surfaced on the read — not a second copy of the rule, and not a relaxation of it: the service still refuses the write, and the client check remains communication only.
  - **`D75` widens that list by one case and one only**: a `COMPLETED` run whose `occurrence_date` is the pantry's today, read-only. It is a `WHERE` rather than a flag because these runs are genuinely a different set — terminal, and on the screen to be read — where a lapsed run is still work. The date comes from `app_config.timezone` inside the query, never the server's clock.
  - **`D66` puts the same flag on every run in the picker's list**, so a lapsed run can be demoted to its own band instead of offered as something to weigh. It is one SQL fragment shared by the single-shift read and the list query, precisely so the two cannot drift. **The list query gained no `WHERE`**: filtering lapsed runs *out* would strand them, because `receiveDone` is deliberately **not** window-gated and the picker is its only route. The window decides how a run is presented, never whether it exists.
  - **`D77` uses the window as a `WHERE` for donations, and the difference from runs is the point.** Both lists in the picker's donation panel — `SUGGESTED` and `CONFIRMED` — are filtered by the same predicate, now factored into `receiverWindowOpen()` in `services/donation.ts` and read by three call sites: the `editable` column, this filter, and `requireEditable`'s write-side refusal. **Filtering here strands nothing**, which is exactly why it is allowed where it was not for runs: an unconfirmed suggestion is hard-deleted at receive-done or by the daily sweep (`I17`) rather than waiting on someone to find it, and a lapsed confirmed row is the Reporter's from S3.1. A run, by contrast, has one route to its own closure and it is that screen.
    - **`CONFIRMED` is window-bounded rather than day-bounded**, and it was day-bounded first. `received_date` on a driver's flag is its *shift's* date, so a suggestion weighed off any other day fell straight out of a today-bounded list at the moment it was weighed. The two lists are the same span so that acting on a row never removes it from the screen.
    - **The counts are the lengths of the lists they head.** `readDonationSummary` was two round trips against a non-serializable reader — a walk-in recorded between them would have been counted and not listed. It is one call now, with the weight summed in SQL beside the rows (`numeric` never becomes a float, `I13`'s reason one layer up).
