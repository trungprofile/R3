...
| 2 | **Donor management** — Admin maintains permanent store donors (master data), including the two things a driver needs to find the door: an optional **map link** (blank means one is derived from the address) and an optional **store photo** (`D20`). Neither is PII: PII in R3 is phone/address on a *person*, and a store's address is where a driver is going. |
...
| 17 | **Category management** — Admin maintains the list of weight-entry categories (master data): add a category, archive one (hidden from new entry, preserved in history/reports), or delete one that has never been used. Removal is I21's rule, not a category-specific one: referencing history ⇒ archive, none ⇒ hard delete. Seeded with the 11 AGFP categories at launch; not a fixed enum. Since `D26` the NTFB list each of them reports under, and its storage requirement, ship seeded too. **`Trash` ships archived**: `D27` computes it from the other categories on the way out, so weighing into it would double-count. |
...
| 6 | **Shift claim & assignment** — A driver self-selects an open shift (primary path), or Staff assigns/defaults a driver (fallback); both set the same owner. Self-select is gated by eligibility (no overlap with the driver's other owned shifts or declared availability); staff-assign is **not** gated — Staff gets a warning and explicit confirmation if the assignment conflicts, but can proceed anyway (and, since `D74`, a **separate** confirmation when the assignment replaces a driver who already had the run: a conflict and a swap are different questions and either, both or neither may need asking), and the shift is flagged so the driver sees the conflict and can raise it with Staff. Claiming a recurring shift once covers all current and future instances that pass the same eligibility gate as a single claim — an instance that would conflict with the driver's own overlapping shift or declared availability is skipped, not force-claimed, and the driver sees which instances didn't go through; releasing a range of them (see cap 8) returns just that range to the board. Open shifts are claimed atomically (no double-claim). |
...
| 15 | **Report generation** — Auto-calculated, inspectable to store-category-day, with in-app AGFP→NTFB (Meal Connect) category mapping, exportable. The mapping is maintained in **Admin** under `tier: 'ADMIN'` (`D17`, superseding `D11`); the report itself remains `report`-duty and is **not** tier-restricted. **Export is a printed receipt, not a file** (`D29`, superseding `D16`'s CSV-plus-print pairing and `D13`'s column list). Meal Connect is a web form with no import, and screenshots of its three entry screens settled the shape: one receipt per `(pickup date, donor)`, then line items of category, storage and pounds. The export mirrors that form so a reporter reads one card and types it in. **The CSV is removed** — one export path cannot disagree with itself. Two things it does that a table could not: it carries **every note** from coordinator, driver and receiver, so the reporter can judge what belongs on the submission; and it emits receipts for pickups that produced **nothing**, ticking the portal's own `Scheduled Pickup Not Attempted` and `No Pounds`, which previously produced no export row at all and were therefore invisible to the food bank. **Trash is computed** on the way out (`D27`, `domain-modeling.md §5.4`), never weighed by a receiver. **The report also tracks what has been filed** (`D35`): the portal takes one submission at a time, so each receipt carries a check-off recording that it went in, who filed it and when — one row per `(pickup date, donor)`, so a second reporter sees the same answer and two people cannot both claim it. It records a filing; it changes no total. **The window is a date range, defaulting to this week** (`D41`), not a fixed week — the same range control S3.2 uses. Every entry is editable **from the receipt it appears on** (`D54`, superseding the drill-in this capability was originally written around; the operation is unchanged, only where it is reached from). **Weight corrections** use the same void-old + insert-new mechanism as receiver edits (cap 14), preserving the audit trail. The **reportable flag** is a plain field edit — last write wins, stamped with who and when — since it carries no weight and has no prior value worth preserving as a row. Before the receiver's edit window closes (`N` days from shift start, ops-configured), the receiver can also edit their own entries directly at the tablet (cap 14); after it closes, editing is Reporter-only, exclusively from here — the only path to correct a bad entry discovered later without reopening receiver access. **A donation that was never flagged for reporting is reachable too** (`D56`): it has no receipt at all, since the report union excludes it, so it is listed beside the receipts with the switch that puts it in — which is the only way this capability's "editing" can include a row the report cannot see. **And a walk-in recorded under a typed-in store name can be pointed at a real store** (`D72`), which is what makes it *fileable* rather than merely reported: `I16(b)` accepts a typed name as a source, so such a row always counted toward the totals, but a Meal Connect receipt keys on a donor row and nothing ever re-pointed the name at the store once that store existed in our list. Those pounds sat outside the food bank permanently, and this capability's "exportable" did not reach them. *(The drill-in was the letter of this capability and turned out to defeat its purpose: a reporter who read the totals screen as somebody else's job never opened it, and reported the edit as missing. `D54` moved it to where they already stand.)* |
| 16 | **Metrics view** — Admin sees per-store and total-intake metrics, including unreported donation volume. Also surfaces shift-coverage failures: unclaimed shifts (window passed, never claimed) and no-shows (claimed, never started), by driver/store/period — the same "consistently under-covered route" or "this driver flakes" visibility the store-donation pattern gets. **Unreported volume is shown as the difference between the two headline figures rather than as a column of its own** (`D58`): the pantry's reading was that a third column plus a period-over-period trend was analysis nobody asked for. The aggregate stays on the payload, so the number this capability names is still served; what went is a rendering of it, on `D34`'s precedent that a derivable third figure is not a third figure. The **previous-period comparison was dropped outright** — it cost a second query over the whole intake union to render one word — so "consistently under-donating" is now read across ranges by hand rather than labelled for you. |
